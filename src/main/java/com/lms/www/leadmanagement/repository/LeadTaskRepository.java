...
public interface LeadTaskRepository extends JpaRepository<LeadTask, Long> {
    ...
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"lead", "lead.assignedTo", "lead.createdBy", "assignedTo", "createdBy"})
    @Query("SELECT t FROM LeadTask t JOIN t.lead l LEFT JOIN l.assignedTo a LEFT JOIN l.createdBy c LEFT JOIN t.assignedTo ta WHERE (:userIds IS NULL OR (a.id IN :userIds OR (a IS NULL AND c.id IN :userIds) OR ta.id IN :userIds)) AND (CAST(:start AS date) IS NULL OR t.dueDate >= :start) AND (CAST(:end AS date) IS NULL OR t.dueDate <= :end)")
    ...
    @Query("SELECT COUNT(t) FROM LeadTask t LEFT JOIN t.lead l LEFT JOIN l.assignedTo a LEFT JOIN l.createdBy c LEFT JOIN t.assignedTo ta WHERE " +
            ...
            "AND t.status NOT IN (com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.COMPLETED, com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.CANCELLED) " +
            "AND t.dueDate < :now")
    long countOverdueTasks(@Param("userIds") Collection<Long> userIds, @Param("now") LocalDateTime now);

    @Query("SELECT COUNT(t) FROM LeadTask t WHERE t.status NOT IN (com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.COMPLETED, com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.CANCELLED) " +
            "AND t.dueDate < :now")
    long countGlobalOverdueTasks(@Param("now") LocalDateTime now);

    @Query("SELECT COUNT(t) FROM LeadTask t LEFT JOIN t.lead l LEFT JOIN l.assignedTo a LEFT JOIN l.createdBy c LEFT JOIN t.assignedTo ta WHERE " +
            "(:userIds IS NULL OR (a.id IN :userIds OR (a IS NULL AND c.id IN :userIds) OR ta.id IN :userIds)) " +
            "AND t.status NOT IN (com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.COMPLETED, com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.CANCELLED) " +
            "AND t.dueDate >= :now AND t.dueDate <= :dayEnd AND (:type IS NULL OR t.taskType = :type)")
    long countTasksDueTodayByType(@Param("userIds") Collection<Long> userIds, @Param("now") LocalDateTime now, @Param("dayEnd") LocalDateTime dayEnd, @Param("type") String type);

    @Query("SELECT COUNT(t) FROM LeadTask t WHERE t.status NOT IN (com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.COMPLETED, com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.CANCELLED) " +
            "AND t.dueDate >= :now AND t.dueDate <= :dayEnd AND (:type IS NULL OR t.taskType = :type)")
    long countGlobalTasksDueTodayByType(@Param("now") LocalDateTime now, @Param("dayEnd") LocalDateTime dayEnd, @Param("type") String type);
    ...
    long countOverdueTasksByType(@Param("userIds") Collection<Long> userIds, @Param("type") String type, @Param("startOfToday") LocalDateTime startOfToday);

    @Query("SELECT COUNT(t) FROM LeadTask t WHERE t.status NOT IN (com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.COMPLETED, com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.CANCELLED) " +
            "AND t.taskType = :type " +
            "AND t.dueDate < :startOfToday")
    long countGlobalOverdueTasksByType(@Param("type") String type, @Param("startOfToday") LocalDateTime startOfToday);
    ...
    @Modifying
    @Query("UPDATE LeadTask t SET t.status = com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.CANCELLED, t.updatedAt = CURRENT_TIMESTAMP WHERE t.lead.id = :leadId AND t.status IN (com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.PENDING, com.lms.www.leadmanagement.entity.LeadTask$TaskStatus.OVERDUE)")
