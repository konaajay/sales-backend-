...
public interface AttendanceSessionRepository extends JpaRepository<AttendanceSession, Long> {
        ...
        @Query("""
                        ...
                        """)
        List<AttendanceSession> _findActiveSessionLocked(
        ...
        default Optional<AttendanceSession> findActiveSession(Long userId, List<AttendanceStatus> statuses) {
                return _findActiveSessionLocked(userId, statuses, PageRequest.of(0, 1)).stream().findFirst();
        }
        ...
        @Query("""
                            ...
                            ORDER BY s.checkInTime DESC
                        """)
        List<AttendanceSession> _findSessions(
                        @org.springframework.data.repository.query.Param("userId") Long userId,
                        @org.springframework.data.repository.query.Param("statuses") List<AttendanceStatus> statuses,
                        Pageable pageable);

        default List<AttendanceSession> findAllByUserIdAndStatusIn(Long userId, List<AttendanceStatus> statuses) {
                return _findSessions(userId, statuses, Pageable.unpaged());
        }

        default Optional<AttendanceSession> findByUserIdAndStatusIn(Long userId, List<AttendanceStatus> statuses) {
                return _findSessions(userId, statuses, PageRequest.of(0, 1)).stream().findFirst();
        }

        default Optional<AttendanceSession> findFirstByUserIdAndStatusInOrderByCheckInTimeDesc(Long userId, List<AttendanceStatus> statuses) {
                return _findSessions(userId, statuses, PageRequest.of(0, 1)).stream().findFirst();
        }

        default List<AttendanceSession> findLatestStatusNoLock(Long userId, List<AttendanceStatus> statuses, Pageable pageable) {
                return _findSessions(userId, statuses, pageable);
        }
        ...
        @Query("""
                            ...
                            WHERE (:isGlobal = true OR s.user.id IN :userIds)
                            AND (s.user.joiningDate IS NULL OR s.user.joiningDate <= :end)
                            AND (s.checkInTime <= :end
                            AND (s.checkOutTime IS NULL OR s.checkOutTime >= :start))
                        """)
        long countPresentUsers(
                        @org.springframework.data.repository.query.Param("isGlobal") boolean isGlobal,
        ...
        List<AttendanceSession> _findFirstByUserIdOrderByCheckInTimeDesc(
        ...
        @Query("""
                            ...
                            AND (s.user.joiningDate IS NULL OR s.checkInTime >= s.user.joiningDate)
                        """)
        List<AttendanceSession> findAllByStatusIn(
                        @org.springframework.data.repository.query.Param("statuses") List<AttendanceStatus> statuses);
}